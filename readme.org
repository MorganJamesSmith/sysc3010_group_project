...
forbidden access for 14 days (unless a system admin uses the GUI to
return their access). Every temperature reading is sent to the server
along with the users ID.
